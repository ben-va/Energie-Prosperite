...
# PARAMETERS
    ...
    tau_e     =p[12]      #
    eta_m     =p[13]      # energy efficiency in consumption
...
# FUNCTIONS
    ...
    #
    util = (gamma_e/gamma_e-1)*(eta_m+1/gamma_m)*((u[2]*sigma_m)/sigma_e)
    ...
    #
    f_u = (gamma_e/(gamma_e-1))*(eta_m*(1-f_tau_m))*(1-f_tau_e+1/gamma_m)*((u[2]*sigma_m)/sigma_e)

    #
    q_e = ((eta_m*(1-f_tau_m)*(1-f_tau_e+gamma_m)*u[2]*sigma_m)/((1-gamma_e)*sigma_e*f_u))

    #
    f_upsilon = gamma/(1-f_u)-gamma

    #
    i = ((u[1]/(u[1]+eta_m))*f_upsilon+(eta_m/(u[1]+eta_m))*tau_m*(f_m*(u[3]+gamma_m*u[1]))-1)
...
parameters = [alpha; beta; delta_m; delta_e; f_m; f_e; gamma_m; gamma_e;
              sigma_m; sigma_e; tau_m; tau_e; eta_m; r_b; phi; gamma;
